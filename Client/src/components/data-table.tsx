...
function RowSelectCheckbox({ row }: { row: Row<z.infer<typeof schema>> }) {
  const selected = row.getIsSelected()
  const [pending, setPending] = React.useState<boolean | null>(null)
  const checked = pending ?? selected
...
function SelectAllCheckbox({
  ...
  table: TanStackTable<z.infer<typeof schema>>
}) {
  const allSelected = table.getIsAllPageRowsSelected()
  const someSelected = table.getIsSomePageRowsSelected()
  const derivedChecked = allSelected || (someSelected && "indeterminate")
  const [pending, setPending] = React.useState<
    boolean | "indeterminate" | null
  >(null)
  const checked = pending ?? derivedChecked
